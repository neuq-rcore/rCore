...
| 位坐标 | 63 ~ 60            | 59 ~ 44        | 43 ~ 0               |
|:------:|:------------------:|:--------------:|:--------------------:|
| 定义   | MODE               | ASID           | PPN                  |
| 含义   | CPU 的页表实现方式 | 地址空间标识符 | 根页表所在的物理页号 |
| 位数   | 4                  | 16             | 44                   |
...
`UserSpace` 结构体用于管理用户程序的加载，它目前只有一个函数：

- **from_elf(elf_data: &[u8])**: 将可执行文件 `elf_data` 加载到用户空间，由于函数体内部复杂，下面分条说明：

  1. 初始化用户空间，并且映射用户空间的 `trampoline` 。

  2. 使用 `xmas_elf` 库将 u8 类型的 `elf_data` 包装为 `Elf_File` 类型，使用 `assert!` 宏检查 ELF 文件 [^1] 的合法性。

  3. 逐项遍历 `elf_data` 的 **sections** ，构建其内存映射。

  4. 创建用户栈空间（ RWU 标记 ）。

  5. 创建异常处理空间（ RW 标记 ）。

模块中顶级函数 **init()** 初始化了堆内存管理和物理页帧分配器。

[^1]: ELF 文件的前 4 个字节分别是 **0x7F, 0x45(E), 0x4C(L), 0x46(F)** ，可以使用 `xxd` 工具查看。
